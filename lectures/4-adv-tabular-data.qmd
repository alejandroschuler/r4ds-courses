...
```{r}
nhanes_tidy = nhanes |>
  select(id, sex, race, bp_sys_1:bp_sys_4) |>
  pivot_longer(
    bp_sys_1:bp_sys_4, 
    names_to="reading",
    values_to="systolic"
...
wide_bp = tibble(
  patient = c(1, 2, 3, 4),
  bp_before = c(152, 141, 158, 147),
  bp_after  = c(138, 135, 141, 149),
)
wide_bp
```
...
```{r, tidy = FALSE}
wide_bp |>
  mutate(bp_change = bp_after - bp_before) |>
  select(patient, bp_change)
```
...
long_bp =
  wide_bp |>
  pivot_longer(-patient, names_to = "time", values_to = "bp", names_prefix = "bp_")
...
## Exercise: one number per participant {data-background-color=teal}

I want a single blood pressure number for each participant instead of four separate readings. Using the NHANES data, produce a table with one row per participant and a column holding the average of that participant's systolic readings. Most participants are missing the fourth reading, so ignore `NA`s when you take the average. (This is exactly what NHANES itself reports as the participant's systolic blood pressure.)

*Hint: the readings live in four separate columns, and `mean()` only knows what to do with one vector at a time. Get them into a single column first.*
...
- The **exam** data has one row per participant per exam component (body measures, blood pressure, and so on), with an id for that exam, the session it was done in, and the examination center it was done at.
