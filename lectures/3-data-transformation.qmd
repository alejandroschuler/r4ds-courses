...
## Why would you want a vector?

Most of the functions that actually calculate something want a plain vector, not a table.
...
:::
:::{.column}
```{r, tidy=F, warning=TRUE}
bmi = nhanes |> select(bmi)
mean(bmi, na.rm=TRUE)
```
...
- the code is identical apart from `pull()` vs `select()`, but only one of them gives you a number
- `mean()` has no idea what to do with a table, so it warns you and hands back `NA`
- watch out: this one *warns* rather than erroring. `median()` and `sd()` would stop with an error instead. A wrong answer that keeps going is more dangerous than a crash
