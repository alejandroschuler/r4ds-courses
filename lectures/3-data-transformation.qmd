...
- the two are identical apart from `pull()` vs `select()`, but only one of them gives you a number
- most functions that actually calculate something want a plain vector. `mean()` has no idea what to do with a table, so it warns and hands back `NA`
- note that it *warns* rather than erroring. `median()` and `sd()` would stop with an error instead. A wrong answer that keeps going is more dangerous than a crash
...
I want a single blood pressure number for each participant instead of four separate readings. Using the NHANES data, create a new column containing the average of the four systolic readings. Most participants are missing the fourth reading, so ignore `NA`s when you take the average. (This is exactly what NHANES itself reports as the participant's systolic blood pressure.)

**Problem 2** 

Produce a plot showing the first systolic reading vs the first diastolic reading for participants aged 60 and over only, and assign color based on whether or not the participant is obese (BMI of 30 or more).
