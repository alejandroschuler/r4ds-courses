...
- Without using the internet, think of how you can use `filter` multiple times to create a dataset where BMI is in the obese range (>=30) **and** systolic blood pressure is high (>=140)

- Using any resources you like, figure out how to use `filter` to create a dataset where **either** BMI is in the obese range (>=30) **or** systolic blood pressure is high (>=140)

## Logical conjunctions (AND)
```{r}
filter(nhanes, bmi >= 30 & bp_sys_1 >= 140)
```
...
```{r}
arrange(nhanes, education, bp_sys_1)
```


## Exercise: top 5 highest blood pressures {data-background-color=teal}

Use `arrange()` and `filter()` to get the data for the 5 rows with the highest first systolic blood pressure readings
...
```{r}
nhanes_by_sys = arrange(nhanes, desc(bp_sys_1)) # using a temporary variable
filter(nhanes_by_sys, row_number()<=5)
```

- we can also accompish this by "nesting" the function calls:
```{r}
filter(arrange(nhanes, desc(bp_sys_1)), row_number()<=5) # "nesting" the calls to filter and arrange
```
...
# same as:
#   filter( arrange(nhanes, desc(bp_sys_1)), row_number()<=5)
```
