...
path_patient <- "data/patients.csv"
...
taking_med_389221 <- df_medication %>%
  filter(CODE == 389221)
...
n_taking_med_389221 <- taking_med_389221 %>%
  select(PATIENT) %>%
  distinct() %>%
  nrow()
...
ggplot(n_taking_top_10_meds, aes(x = medication, y = n_patients)) +
  geom_bar(stat = "identity", fill = "steelblue") +
  labs(x = "Medication Code", y = "Number of Patients",
...
df_patient <- df_patient %>%
  mutate(`age (years)` = as.numeric(floor(ref_date - BIRTHDATE)) / 365.25)
```
...
med_counts_under_18 <- left_join(under_18, med_counts, by = "Id") %>%
  select(Id, med_count)
```
...
highly_medicated_minors <- med_counts_under_18 %>%
  filter(med_count >= 8)

# TODO: Count the number of minors taking at least 8 medications
n_highly_medicated_minors <- nrow(highly_medicated_minors)
