...
df_patient$age <- difftime(ref_date, df_patient$BIRTHDATE, units = "days") / 365.25
...
df_patient <- rename(df_patient, "age (years)" = "age")
```
...
ggplot(df_patient, aes(x = `age (years)`)) +
...
under_18_or_over_65 <- patient_ages %>%
...
between_18_and_65 <- patient_ages %>%
  filter(`age (years)` >= 18) %>%
  filter(`age (years)` <= 65)
...
age_group_averages <- patient_ages %>%
  group_by(age_group) %>%
  summarize(
